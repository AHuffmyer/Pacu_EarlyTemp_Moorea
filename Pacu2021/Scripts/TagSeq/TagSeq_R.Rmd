...
allgenesfilt_PCA <- ggplot(gPCAdata, aes(PC1, PC2, color=Temperature, shape=Date, group=Temperature, fill=Temperature)) + 
  geom_point(size=3) + 
  coord_fixed()+
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
...
NEED TO RUN THESE ANALYSES WHERE WE PARTITION BY DAY - RUN 31 vs 27 on 12/05 and 12/06 and run 34 vs 27 on 12/07
...
```
There are 348 differentially expressed genes between 31 and 34. 
...
ADD HEATMAP THAT SHOWS TEMPERATURE AND DAY BLOCKS 