...
Analysis of TagSeq dataset for Pocillopora acuta Moorea early life history temperature exposure.  
...
# **Conduct PERMANOVA by date and temperature**  
...
test$Temperature<-metadata$Temperature[match(test$Sample, metadata$SampleName)]
test$Date<-metadata$Date[match(test$Sample, metadata$SampleName)]
...
scaled_test <-prcomp(test[c(1:8221)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test)

# scale data
vegan <- scale(test[c(1:8221)])

# PerMANOVA 
permanova<-adonis2(vegan ~ Temperature*Date, data = test, method='eu')
permanova
```
