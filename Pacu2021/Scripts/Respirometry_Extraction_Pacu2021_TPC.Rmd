...
Temp.Inc <- Temp.Inc %>%
  group_by(TemperatureCat) %>%
  summarise(Temperature=mean(Measured.Temp))

#join the sample info with the mean temperatures measured in the incubator for each run
Sample.Info <- left_join(Sample.Info, Temp.Inc)
...
remove<-c("2 A1", "2 C1", "3 A4", "4 A3", "4 B5", "4 C5", "4 D1", "7 A3","7 B2", "7 C2", "7 C4", "8 B3", "8 B5", "8 C3", "8 D1", "8 D4", "B A2", "C A1", "C A2", "C C3")
...
Account for volume and substract blank values.    
...
meanblank <- mean(blank_data$R.umol.min) #mean R phase blanks

par(mfrow=c(2,2))
plot(as.factor(blank_data$Run),blank_data$R.umol.min, ylab="umol O2 min-1", xlab="RUN Number")
abline(h=-meanblank)
#blanks during dark phase
plot(as.factor(blank_data$Plate), blank_data$R.umol.min, ylab="umol O2 min-1",, xlab="Plate Number")
abline(h=-meanblank)#blanks during dark phase
plot(as.factor(blank_data$Temperature), blank_data$R.umol.min, ylab="umol O2 min-1",xlab="Temperature Level")
abline(h=-meanblank)#blanks during dark phase
plot(as.factor(blank_data$Treatment), blank_data$R.umol.min, ylab="umol O2 min-1", xlab="Treatment")
abline(h=-meanblank)#blanks during dark phase

#identify the mean blank of all runs' blanks
oxygen$R.Blank.umol.min <- meanblank

#CODE FOR CALCULATING MEAN BY RUN
#aggregate(R.umol.min ~ Date*Run, data=blank_data, mean) #calculate average blank for each run
