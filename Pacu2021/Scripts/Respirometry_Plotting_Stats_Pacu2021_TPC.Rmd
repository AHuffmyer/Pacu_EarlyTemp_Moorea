...
#PRdata<-read.csv("Pacu2021/Output/Respiration/TPC/oxygen_Resp_rates1.csv") #load data

#With SDR calculated temps
PRdata<-read.csv("Pacu2021/Output/Respiration/TPC/oxygen_Resp_rates.csv") #load data
```

Load temperatures calculated as the average of temperatures measured by the SDR plates. These are the temperatures that will be used for all analyses below.    

```{r}
#load actual temps data generated from extraction script  
#actual_temps<-read.csv("Pacu2021/Output/Respiration/TPC/actual_temps.csv", header=TRUE, sep=",")

#format run column to match PRdata 
#actual_temps$Run<-gsub("Run","",as.character(actual_temps$Run))
#actual_temps$Run<-as.factor(actual_temps$Run)
```
...
#pull in temperature data for actual temperatures  
#PRdata$code<-paste(PRdata$Date, PRdata$Run)
#actual_temps$code<-paste(actual_temps$Date, actual_temps$Run)
#PRdata <- left_join(PRdata, actual_temps)

#PRdata$Temperature<-actual_temps$temp[match(PRdata$code, actual_temps$code)]
...
Filtering 
```{r}
levels(as.factor(PRdata$Temperature))

#For Filtering Temp Cat
# PRdata <- PRdata %>%
...
#For Filtering SDR Temps
 PRdata <- PRdata %>%
    filter(!Temperature<20) %>%
     filter(!Temperature>32.5)
...
max(PRdata$rate)
which(PRdata$rate ==max(PRdata$rate))
PRdata <- PRdata %>% filter(rate < max(PRdata$rate))
max(PRdata$rate)
which(PRdata$rate ==max(PRdata$rate))
PRdata <- PRdata %>% filter(rate < max(PRdata$rate))
#PRdata <- PRdata[-c(100,107),]
...
ggsave("Pacu2021/Figures/FigureX_TPC_estimates.pdf", figure, dpi=300, w=6, h=8, units="in")
ggsave("Pacu2021/Figures/FigureX_TPC_estimates.jpg", figure, dpi=300, w=6, h=8, units="in")
